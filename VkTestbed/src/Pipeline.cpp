...
Pipeline::Pipeline(const std::string& vertPath, const std::string& fragPath, Device& device, const PipelineConfigInfo& configInfo)
	: m_Device(device)
{
...
void Pipeline::CreateGraphicsPipeline(const std::string& vertPath, const std::string& fragPath, const PipelineConfigInfo& configInfo)
{
	...
	CreateShaderModule(vertSrc, &m_VertexShaderModule);
	CreateShaderModule(fragSrc, &m_FragmentShaderModule);
	...
	VK_CHECK(vkCreateGraphicsPipelines(m_Device.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_GraphicsPipeline), "Failed to create graphics pipeline!");
}

void Pipeline::CreateShaderModule(const std::vector<char> src, VkShaderModule* shaderModule)
{
	...
	VK_CHECK(vkCreateShaderModule(m_Device.device(), &createInfo, nullptr, shaderModule), "Failed to create shader module");
}
